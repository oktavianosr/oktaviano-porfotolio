...
export function useVisitor(): UseVisitorReturn {
  ...
  useEffect(() => {
    try {
      ...
      console.warn("Could not read visitor data from Session", error);
    } finally {
  ...
  const saveVisitor = (data: Omit<StoredVisitor, "visitedAt">): void => {
    ...
    try {
      // Simpan ke sessionStorage sebagai JSON string
      sessionStorage.setItem(STORAGE_KEY, JSON.stringify(visitorWithTimestamp));
      setVisitor(visitorWithTimestamp);
    } catch (error) {
      console.warn("Could not save visitor data to Session:", error);
